...
namespace Bhell.Systems {
    ...
    public class SplineMovimentSystem : JobComponentSystem {
        ...
        struct UpdateSplineWithParent : IJobChunk {
            public ComponentTypeHandle<LocalToParent> localToParentType;
            public ComponentTypeHandle<SplineAnimationSpeed> splineAnimationType;
            [ReadOnly] public ComponentTypeHandle<JustSpawned> justSpawnedType;
            ...
            public void Execute(ArchetypeChunk chunk, int index, int entityOffset) {
                ...
                for (int i = 0; i < chunk.Count; i++) {
                    ...
                    if (justSpawnedAcessor.IsCreated) move = anim.speed * justSpawnedAcessor[i].deltaTime;
                    float left;
        ...
        protected override JobHandle OnUpdate(JobHandle dependencies) {
            JobHandle handle = new UpdateSplineWithParent {
                localToParentType = GetComponentTypeHandle<LocalToParent>(),
                splineAnimationType = GetComponentTypeHandle<SplineAnimationSpeed>(),
                justSpawnedType = GetComponentTypeHandle<JustSpawned>(),
